...
- Endpoints:
  ...
  - `GET /datastreams/{id}/schema` (Stage 33) — returns the stored SWE Common DataRecord JSON schema for observation value encodings. Stored under gateway-local `cs-api.datastream.schema` because semstreams beta.88 intentionally scope-cuts Datastream schema storage from `vocabulary/csapi`; the target pattern is a first-class SWE schema artifact entity with its own `StorageRef`, related from the Datastream by a CS API vocabulary predicate.
