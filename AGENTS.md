...
- Endpoints:
  ...
  - `GET /datastreams/{id}/schema` (Stage 33) — returns the stored SWE Common DataRecord JSON schema for observation value encodings. Stored under gateway-local `cs-api.datastream.schema`. Semstreams beta.91 closed #182 with dotted CS API relationship predicates plus `*IRI` boundary constants; Stage 41 wires the ObjectStore + helper for typed `csapi:SWESchemaDocument` artifacts, leaving the Datastream/ControlStream handler call-site migration as the remaining local bridge retirement.
