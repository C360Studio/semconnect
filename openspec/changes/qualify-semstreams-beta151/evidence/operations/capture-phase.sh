...
"$docker" exec semconnect-conformance-nats-1 \
  wget -qO- 'http://semstreams-backend:9090/metrics' >"$raw_dir/backend-metrics.prom"
/usr/bin/grep -E 'semstreams_graph_ingest_(mutation_rejections|predicate_contract_rejections|entity_state_contract_rejections|foreign_edge_unclaimed|foreign_edge_dropped)_total' \
  "$raw_dir/backend-metrics.prom" >"$phase_dir/structural-foreign-metrics.prom" || true

backend_started=$($docker inspect semconnect-conformance-semstreams-backend-1 --format '{{.State.StartedAt}}')
$docker logs --since "$backend_started" --timestamps \
  semconnect-conformance-semstreams-backend-1 >"$raw_dir/backend-window.log" 2>&1
