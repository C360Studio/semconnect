...
- Endpoints:
  ...
  - `GET /datastreams/{id}/schema` (Stage 33) — returns the stored SWE Common DataRecord JSON schema for observation value encodings. Stored under gateway-local `cs-api.datastream.schema`. Semstreams beta.90 closed #171 with the typed-artifact entity pattern and CS API artifact classes, but #182 tracks the remaining dotted-predicate constant gap; migrate only when the relationship predicates preserve SemStreams' three-level dotted internal predicate contract.
