...
- Endpoints:
  ...
  - `GET /feasibility` (Stage 55) — CS API Part 2 Command Feasibility collection. Predicate-query on gateway-local `FeasibilityTypeIRI`, then batch-hydrates entities into read-side Feasibility resources. Command feasibility evaluation/execution remains out of scope at v0.1.
  ...
  - `POST /feasibility` (Stage 55) — JSON fixture helper used by the conformance harness to create read-side Feasibility metadata. Stores gateway-local three-part dotted predicates (`cs-api.feasibility.controlstream`, `cs-api.feasibility.status`, `cs-api.feasibility.params`, `cs-api.feasibility.result`) until semstreams grows canonical CS API Feasibility vocabulary terms.
...
- **Stage 6 + 9 conformance harness** (`conformance/run.sh` + `.github/workflows/conformance.yml`)
  ...
  **Current outcome (2026-07-06 SemStreams pin refresh): `total=137 passed=137 failed=0 skipped=0`.**
  ...
  The Feasibility type/predicates are gateway-local until semstreams grows canonical
  CS API Feasibility vocabulary terms.
...
| `GET /feasibility` / `GET /collections/all_feasibility/items` | `graph.index.query.predicate` (rdf:type = gateway-local `FeasibilityTypeIRI`) → `graph.query.batch` hydration → JSON Feasibility collection. Stage 55. |
...
| `GET /controlstream/{id}/feasibility` | Parent ControlStream `graph.query.entity` kind check → predicate-query all Feasibility resources → `graph.query.batch` hydration → filter by gateway-local `cs-api.feasibility.controlstream`. Stage 55; command execution/evaluation remains out of scope. |
