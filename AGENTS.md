...
- Endpoints:
  ...
  - `POST /systems` (Stage 8; Stage 14 added position preservation; Stage 16 added JSON Feature body; Stage 18 added uid preservation; Stage 29 moved uid/position to semstreams beta.79 native predicates) — accepts four request media types: `application/sml+json` / `application/sensorml+json` (SensorML path; semstreams emits `uniqueId` and `position` triples natively) and `application/json` / `application/geo+json` (CS API §7.6 GeoJSON Feature body — Stage 16; entity ID minted from `properties.uid`, the uid is preserved on `sensorml.process.uid`, optional `geometry` round-trips via `sensorml.process.position`). `Content-Type` selects the branch (`buildSystemTriplesFromSensorML` vs `buildSystemTriplesFromFeature`); both feed the shared `ingestTriples` publish path. Returns 201 Created with Location. Stage 30 pins semstreams beta.87, which includes entity mutation subjects, post-write read-back semantics, and header-classified request/reply errors; migrating POST from `graph.mutation.triple.add_batch` to `graph.mutation.entity.create_with_triples` is now local semconnect follow-up work.
  - `PUT /systems/{id}` (Stage 16) — CS API §7.6 create-replace-delete replace semantics. Accepts `application/json` / `application/geo+json` only (no SensorML on PUT — the lossy reverse-mapping would surprise clients on read-back). Verifies the body's `properties.uid` mints to the same entity ID as the path; mismatch yields 400 *before* any destructive remove. Replace is implemented as `deleteAllEntityTriples` then `ingestTriples` — N round-trips per call until semconnect migrates to the beta.87 entity mutation subjects. Returns 204 No Content.
  ...
  - `GET /datastreams/{id}/schema` (Stage 33) — returns the stored SWE Common DataRecord JSON schema for observation value encodings. Stored under gateway-local `cs-api.datastream.schema` because semstreams beta.88 intentionally scope-cuts Datastream schema storage to a future StorageRef primitive.
  ...
  - `PATCH /datastreams/{id}` (Stage 35) — CS API `conf/update` partial-update parity with `/systems/{id}`. Accepts `application/json` only. Non-empty `name`, `description`, `system`, `observedProperty`, or `schema` fields replace the corresponding triples; absent/empty fields leave existing triples untouched. Body `id` mismatch, invalid system refs, invalid SWE Common schema, and `schema: null` fail before destructive remove. PATCH against a missing entity returns 404 (no upsert — PUT remains the upsert path). Same delete-all + add-batch partial-erasure window as PUT.
  - `DELETE /datastreams/{id}` (Stage 17; Stage 36 purges observation subject) — CS API §10.6. Idempotent (errEntityNotFound swallowed → 204). Removes entity triples, then purges messages on the exact JetStream subject `cs-api.observations.{id}`. If graph deletion succeeds but stream purge fails, returns 503 with `X-CS-Partial-Delete: true` and `X-CS-Observation-Purge-Failed: true` so retrying DELETE can finish cleanup.
  ...
  - `GET /systems/{id}/controlstreams` (Stage 24) — system-scoped ControlStream collection, filtered by `vocabulary/csapi.ControlsSystem` (legacy read fallback: `cs-api.controlstream.system`).
  ...
  - `GET /systems/{id}/events` (Stage 25) — normative Requirement 43 system-scoped SystemEvent collection, filtered by `vocabulary/csapi.EventForSystem` (legacy read fallback: `cs-api.systemevent.system`).
...
| `GET /systems/{id}/controlstreams` | Predicate-query all ControlStreams, hydrate, filter by `vocabulary/csapi.ControlsSystem`. Stage 24. |
...
| `GET /systems/{id}/events` | Predicate-query all SystemEvents, hydrate, filter by `vocabulary/csapi.EventForSystem`. Stage 25. |
...
| `POST /datastreams` | JSON decode → optional `pkg/swecommon` schema validation → `datastreamToTriples` → `graph.mutation.triple.add_batch` request/reply via `ingestTriples`. 201 Created + Location. Honors client-supplied 6-part `id`. |
| `PUT /datastreams/{id}` | JSON decode → `datastreamToTriples` → `deleteAllEntityTriples` → `ingestTriples`. 204 No Content. Stage 17. |
| `PATCH /datastreams/{id}` | JSON decode → optional `pkg/swecommon` schema validation → merge non-empty fields over existing triples → `deleteAllEntityTriples` → `ingestTriples`. 204 No Content; 404 if missing (no upsert). Stage 35. |
| `DELETE /datastreams/{id}` | `fetchEntity` → `deleteAllEntityTriples` → JetStream `Purge(WithPurgeSubject("cs-api.observations.{id}"))`. 204 No Content (idempotent). Stage 17; observation purge added Stage 36. |
