...
- Endpoints:
  ...
  - `GET /datastreams` (Stage 8; Stage 44 makes collection items full Part 2 resources; Stage 54 adds Part 2 Advanced Filtering) — predicate-query for `rdf:type = DatastreamTypeIRI` (Stage 13: `csapi.Datastream` from `vocabulary/csapi` since semstreams v1.0.0-beta.75; pre-Stage-13 was a locally-minted HTTPS IRI), then batch-hydrates entities so collection `items` carry system reference, observed properties, phenomenon/result time evidence, formats, resultType, and links. Supports `?observedProperty=`, `?phenomenonTime=`, and `?resultTime=` filters over hydrated resource fields. `X-CS-Datastream-Subset: true` header retired at Stage 13.
  - `GET /datastreams/{id}` (Stage 8; Stage 33 adds schema field/link; Stage 42 moves storage to artifacts; Stage 44 adds Part 2 fields; Stage 54 makes `observedProperties[]` object-shaped) — entity-query → CS API §10/Part 2 Datastream JSON subset (id, name, description, system/system@id/system@link, outputName, observedProperty/observedProperties[].definition, phenomenonTime/resultTime, formats, resultType, optional SWE Common DataRecord schema). If the Datastream has `csapi.HasResultSchema`, the handler follows the typed `csapi:SWESchemaDocument` artifact entity's `StorageRef` to ObjectStore and inlines the canonical schema. 404 if entity exists but is not a Datastream.
  ...
  - `GET /observations` (Stage 44; Stage 45 makes it populated; Stage 54 adds Part 2 Advanced Filtering) — global Observation collection backed by a wildcard JetStream read over `cs-api.observations.>`. JSON resources include Part 2 `datastream@id` recovered from the subject and support `?phenomenonTime=` / `?resultTime=` filters over the returned resource fields; no graph index is introduced at v0.1.
  ...
  - `GET /controlstreams` (Stage 24; Stage 54 adds Part 2 Advanced Filtering) — CS API Part 2 control-stream collection. Predicate-query on `vocabulary/csapi.ControlStream`, then Stage 40 batch hydration so collection `items` are full ControlStream resources. Supports `?controlledProperty=`, `?issueTime=`, and `?executionTime=` filters over hydrated resource fields.
  - `GET /controlstreams/{id}` (Stage 24; Stage 34 makes schema SWE-backed; Stage 54 adds time evidence) — JSON ControlStream subset with system reference, inputName, controlledProperties, issue/execution time evidence, formats, live/async flags, and command links. Stage 47 adds canonical Part 2 alias `GET /controls/{id}` to the same handler.
  ...
  - `GET /commands` (Stage 43; Stage 51 adds read-side Command metadata; Stage 54 adds Part 2 Advanced Filtering) — global Command collection backed by graph `csapi.Command` entities. Supports `?issueTime=`, `?executionTime=`, `?statusCode=`, and `?sender=` filters over stored metadata. Command execution/status lifecycle remains intentionally out of scope at v0.1.
  ...
  - `GET /feasibility` (Stage 55) — CS API Part 2 Command Feasibility collection. Predicate-query on semconnect-owned `csapi.Feasibility`, then batch-hydrates entities into read-side Feasibility resources. Command feasibility evaluation/execution remains out of scope at v0.1.
  - `GET /feasibility/{id}` (Stage 55) — canonical Feasibility item read with `status`, `params` / `parameters`, optional `result`, and ControlStream reference evidence.
  - `GET /feasibility/{id}/status` + `GET /feasibility/{id}/result` (Stage 55) — readable status/result item collections for the Feasibility resource.
  - `GET /controlstream/{id}/feasibility` (Stage 55) — normative singular Part 2 ControlStream-scoped Feasibility collection. Validates the parent ControlStream, then filters Feasibility resources by `controlstream@id`.
  - `POST /feasibility` (Stage 55) — JSON fixture helper used by the conformance harness to create read-side Feasibility metadata. Stores semconnect-owned `vocabulary/csapi` predicates for ControlStream, status, parameters, and result.
  ...
  - `GET /systemEvents` (Stage 25; Stage 54 adds Part 2 Advanced Filtering) — CS API Part 2 SystemEvent collection. Predicate-query on `vocabulary/csapi.SystemEvent`, then Stage 40 batch hydration so collection `items` are full SystemEvent resources. Supports `?eventType=` filtering over hydrated resource fields. Stage 47 also exposes this collection at `/collections/all_system_events/items` for the ETS Part 2 collection-discovery path.
...
- **Stage 6 + 9 conformance harness** (`conformance/run.sh` + `.github/workflows/conformance.yml`)
  ...
  **Qualified beta.153 outcome (2026-07-19): `total=137 passed=137 failed=0 skipped=0`.**
  ...
  product-owned predicate `cs-api.deployment.parent`, closing the parent-scoped
  collection and canonical child item assertions. Stage 51 adds read-side
  Command metadata resources using semconnect-owned `csapi.Command` and
  ...
  reads by reusing the semconnect-owned SensorML SimpleProcess/AggregateProcess
  ...
  Stage 54 declares Part 2 Advanced Filtering and implements the read-only filter
  slice the ETS exercises: Datastream observedProperty/time, Observation time,
  ControlStream controlledProperty/time, Command status/sender/time, and SystemEvent
  eventType. Filtering happens over hydrated resource shapes; no new graph index is
  introduced. Stage 55 declares Part 2 Command Feasibility and adds the read-side
  Feasibility collection/item, status/result subresources, the normative singular
  `/controlstream/{id}/feasibility` scoped endpoint, and a narrow fixture POST helper.
  ADR-S003 later promotes the Feasibility type and predicates into semconnect-owned
  `vocabulary/csapi`.
  ...
  Stage 52 (118/0/19) → Stage 53 (121/0/16) → Stage 54 (130/0/7) →
  Stage 55 (137/0/0).
  ...
  controlstreams, commands, feasibility, systemEvents);
...
| `GET /deployments/{id}/subdeployments` | Parent `graph.query.entity` kind check → predicate-query all Deployments → `graph.query.batch` hydration → filter child Deployments whose owned `cs-api.deployment.parent` relation points at the parent. Stage 50. |
| `POST /deployments` | `buildDeploymentTriplesFromFeature` (json / geo+json) → `ingestTriples`. Optional geometry → `sensorml.process.position` triple; optional `properties.deployedSystems@link[]` hrefs → `cs-api.deployment.deployed-systems`; optional `parent@id` / `parent@link` → owned `cs-api.deployment.parent`. 201 Created + Location. Stage 21; association evidence Stage 46; subdeployment parent evidence Stage 50. |
...
| `POST /samplingFeatures` | `buildSamplingFeatureTriplesFromFeature` (json / geo+json) → `ingestTriples`. Optional geometry → `sensorml.process.position` triple; optional `properties.hostedProcedure@link.href` → `cs-api.samplingfeature.hosted-procedure`. 201 Created + Location. Stage 22; association evidence Stage 46. |
...
| `GET /controlstreams` | `graph.index.query.predicate` (rdf:type = `vocabulary/csapi.ControlStream`) → `graph.query.batch` hydration → optional Stage 54 filters (`controlledProperty`, `issueTime`, `executionTime`) → JSON ControlStreamCollection. Stage 24; batch hydration Stage 40. |
...
| `GET /commands` | `graph.index.query.predicate` (rdf:type = `csapi.Command`) → `graph.query.batch` hydration → optional Stage 54 filters (`issueTime`, `executionTime`, `statusCode`, `sender`) → JSON Command collection. Stage 43; populated read-side metadata Stage 51. Command execution/status lifecycle remains out of scope at v0.1. |
...
| `GET /feasibility` / `GET /collections/all_feasibility/items` | `graph.index.query.predicate` (type = semconnect-owned `csapi.Feasibility`) → `graph.query.batch` hydration → JSON Feasibility collection. Stage 55. |
| `GET /feasibility/{id}` | `graph.query.entity` → Feasibility kind check → JSON Feasibility metadata resource. Stage 55. |
| `GET /feasibility/{id}/status` / `GET /feasibility/{id}/result` | `graph.query.entity` → Feasibility kind check → JSON items collection for status/result metadata. Stage 55. |
| `GET /controlstream/{id}/feasibility` | Parent ControlStream `graph.query.entity` kind check → predicate-query all Feasibility resources → `graph.query.batch` hydration → filter by owned `cs-api.feasibility.controlstream`. Stage 55; command execution/evaluation remains out of scope. |
| `POST /feasibility` | `buildFeasibilityTriples` (json fixture helper) → `ingestTriples`. Required `controlstream@id` → `cs-api.feasibility.controlstream`; optional status, params, and result are metadata triples. Stage 55. No command feasibility evaluation side effect. |
| `GET /observations` | Wildcard JetStream read over `cs-api.observations.>` → optional Stage 54 time filters over normalized JSON resources → JSON ObservationCollection with `datastream@id` recovered from subject. Stage 44 endpoint; populated Stage 45. |
...
| `GET /systemEvents` / `GET /collections/all_system_events/items` | `graph.index.query.predicate` (rdf:type = `vocabulary/csapi.SystemEvent`) → `graph.query.batch` hydration → optional Stage 54 `eventType` filter → JSON SystemEventCollection. Stage 25; batch hydration Stage 40; collection-items bridge Stage 47. |
...
| `GET /datastreams` | `graph.index.query.predicate` (rdf:type = `csapi.Datastream` since Stage 13) → `graph.query.batch` hydration → optional Stage 54 filters (`observedProperty`, `phenomenonTime`, `resultTime`) → JSON DatastreamCollection. |
